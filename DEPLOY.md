...
DATABASE_URL=postgresql+psycopg://...   # Required for commercial/worker deployments
REQUIRE_SERVER_DB=true
...
```bash
# Start API + worker + Postgres together
docker compose up --build
```

Visit `http://localhost:8000` to test.

The default compose stack now uses:
- `postgres` for the shared job database
- `manimations` as the API service
- `manimations-worker` as the background worker
...
2. Create S3 bucket:
   ...
   ```
3. Keep bucket objects private and use signed URLs or protected backend delivery
...
For queue visibility, add an admin user and check:
```bash
curl -H "Authorization: Bearer <admin-token>" \
  https://your-app.example.com/api/v1/system/queue-health
```
