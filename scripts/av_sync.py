...
def _ensure_ffmpeg():
    ...
    if ffmpeg_binary:
        if os.path.isfile(ffmpeg_binary):
            ...
            print(f"[av_sync] WARNING: FFMPEG_BINARY '{ffmpeg_binary}' not found; falling back to PATH")
            ffmpeg_binary = None
    if not ffmpeg_binary:
        which = shutil.which("ffmpeg")
        if which:
            moviepy.config.FFMPEG_BINARY = which
        else:
            raise RuntimeError(
...
def generate_srt_from_plan(plan_json: dict, srt_path: Path, audio_paths: Optional[List[Path]] = None) -> Path:
    """Generate SRT subtitle file from solution plan voiceover data with frame-accurate timestamps.

    Uses scene-level voiceover text and audio durations to produce .srt subtitles
    that stay exactly in sync with the final video.
    """
    scenes = plan_json.get("animation_plan", {}).get("scenes", [])
    ...
    time_offset = 0.0
    intro_path = os.getenv("INTRO_VIDEO_PATH", "intro.mp4")
    if os.getenv("INTRO_ENABLED", "true").lower() != "false" and Path(intro_path).exists():
        try:
            time_offset += _probe_duration(Path(intro_path))
        except Exception:
            time_offset += 5.0
    ...
    for sidx, scene in enumerate(scenes):
        ...
        if not voiceover.strip():
            continue
        dur = scene.get("duration_seconds", 10.0)
        if audio_paths and sidx < len(audio_paths):
            try:
                dur = _probe_duration(audio_paths[sidx])
            except Exception:
                pass
        words = voiceover.split()
        chunk_size = max(1, len(words) // max(1, int(dur // 4)))
        chunks = [words[i:i + chunk_size] for i in range(0, len(words), chunk_size)]
        chunk_dur = dur / max(len(chunks), 1)

        for ci, chunk in enumerate(chunks):
            start = time_offset + ci * chunk_dur
            end = min(time_offset + (ci + 1) * chunk_dur, time_offset + dur)
            if start >= end:
                break
    ...
    srt_path.write_text("\n".join(lines), encoding="utf-8")
    print(f"[av_sync] Generated SRT with {sub_idx - 1} subtitles -> {srt_path}")
    return srt_path
...
def main():
    ...
    parser.add_argument("--plan", help="Path to solution_plan.json for subtitle generation")
    ...
    if args.plan:
        ...
        if plan_path.exists():
            ...
            generate_srt_from_plan(plan, srt_path, audio_paths)
