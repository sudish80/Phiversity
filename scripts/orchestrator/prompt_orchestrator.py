...
def call_deepseek_solver(
  ...
  try:
    ...
    # Skip system prompts for fine-tuned models
    if os.getenv("USE_FINETUNED_MODEL", "false").lower() == "true":
      print("[orchestrator] Fine-tuned model mode: Sending question directly without system prompts")
    ...
    # On DeepSeek failure, try OpenAI, then Gemini, then GROQ, then Ollama before surfacing the error
    if not allow_fallback or os.getenv("ORCHESTRATOR_DISABLE_FALLBACK"):
    ...
    try:
      ...
      try:
        ...
        print(f"[orchestrator] Gemini solver attempt failed — attempting GROQ. Error: {e3}")
        try:
          return call_groq_solver(solver_prompt, context_question=context_question)
        except Exception as e4:
          print(f"[orchestrator] GROQ solver attempt failed — attempting Ollama. Error: {e4}")
          try:
            return call_ollama_solver(solver_prompt, context_question=context_question)
          except Exception as e5:
            print(f"[orchestrator] Ollama solver attempt also failed. Error: {e5}")
            raise
...
def call_gemini_solver(solver_prompt: str, gemini_model: str | None = None, context_question: str | None = None) -> Dict[str, Any]:
  ...
  model_name = gemini_model or os.getenv("GEMINI_MODEL", "gemini-1.5-flash-latest")
...
def call_openrouter_solver(
  solver_prompt: str,
  openrouter_model: str | None = None,
  context_question: str | None = None,
) -> Dict[str, Any]:
  """Call OpenRouter as the solver to produce the final JSON plan."""
  if os.getenv("ORCHESTRATOR_OFFLINE"):
    raise RuntimeError("ORCHESTRATOR_OFFLINE is set; OpenRouter solver disabled")
  client = get_openrouter_client()
  model = openrouter_model or os.getenv("OPENROUTER_MODEL", "meta-llama/llama-3.1-70b-instruct")
  print(f"[orchestrator] Calling OpenRouter solver via model: {model}")
  ...
  content = resp.choices[0].message.content.strip()
  print(f"[orchestrator] OpenRouter solver responded with {len(content)} chars.")
  try:
    return json.loads(content)
  except json.JSONDecodeError:
    start = content.find("{")
...
def call_groq_solver(solver_prompt: str, groq_model: str | None = None, context_question: str | None = None) -> Dict[str, Any]:
  """Call GROQ as the solver to produce the final JSON plan."""
  if os.getenv("ORCHESTRATOR_OFFLINE"):
    raise RuntimeError("ORCHESTRATOR_OFFLINE is set; GROQ solver disabled")
  client = get_groq_client()
  model = groq_model or os.getenv("GROQ_MODEL", "mixtral-8x7b-32768")
  print(f"[orchestrator] Calling GROQ solver via model: {model}")
  ...
  content = resp.choices[0].message.content.strip()
  print(f"[orchestrator] GROQ solver responded with {len(content)} chars.")
  try:
    return json.loads(content)
  except json.JSONDecodeError:
    start = content.find("{")
...
def orchestrate_solution(user_question: str, override_solver_prompt: str | None = None) -> SolverOutput:
  try:
    # For fine-tuned models, skip prompt crafting and send question directly
    if os.getenv("USE_FINETUNED_MODEL", "false").lower() == "true":
      print("[orchestrator] Fine-tuned model mode: Skipping prompt crafting")
      solver_prompt = user_question.strip()
    elif override_solver_prompt and override_solver_prompt.strip():
    ...
    # Prefer Gemini first when available; fall back to other providers on failure.
    if os.getenv("GEMINI_API_KEY"):
      print("[orchestrator] Gemini preferred: attempting Gemini first.")
      try:
        return SolverOutput.model_validate(call_gemini_solver(solver_prompt, context_question=user_question))
      except Exception as e:
        print(f"[orchestrator] Gemini primary attempt failed — falling back. Error: {e}")

    if os.getenv("GROQ_API_KEY") and not os.getenv("DEEPSEEK_API_KEY") and not os.getenv("OPENAI_API_KEY"):
      print("[orchestrator] Detected GROQ-only configuration. Routing directly to GROQ.")
      return SolverOutput.model_validate(call_groq_solver(solver_prompt, context_question=user_question))

    # Prefer DeepSeek; if it fails, the function will attempt OpenAI, then Gemini, then GROQ, then Ollama.
