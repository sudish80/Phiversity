...
# Default timeout for all LLM API calls (seconds). Override via LLM_TIMEOUT env var.
_LLM_TIMEOUT = float(os.getenv("LLM_TIMEOUT", "300"))
...
def get_chatgpt_client(api_key: Optional[str] = None) -> OpenAI:
    ...
    try:
        return OpenAI(api_key=key, timeout=_LLM_TIMEOUT)
    finally:
        if prev is not None:
...
def get_deepseek_client(api_key: Optional[str] = None, base_url: Optional[str] = None) -> OpenAI:
    ...
    return OpenAI(api_key=key, base_url=url, timeout=_LLM_TIMEOUT)
...
def get_nvidia_client(api_key: Optional[str] = None, model_name: Optional[str] = None) -> OpenAI:
    """Return an OpenAI-compatible client for NVIDIA AI Foundation Endpoints.
    ...
    client = OpenAI(api_key=key, base_url=url, timeout=_LLM_TIMEOUT)
    return client
