...
def generate_scene_script(
    ...
    for sidx, sc in enumerate(scenes_with_intro, start=1):
        ...
        # Clear scene at end but preserve watermark to prevent overlapping content
        lines.append("        # Remove all objects except watermark")
        lines.append("        all_except_watermark = [m for m in self.mobjects if m != watermark]")
        lines.append("        if all_except_watermark:")
        lines.append("            self.play(FadeOut(Group(*all_except_watermark)))")
        lines.append("        self.wait(0.3)")
